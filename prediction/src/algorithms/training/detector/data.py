            ...
            if not isRandomImg:
                ...
                if self.phase == 'train' and not isRandom:
                    sample, target, bboxes, coord = augment(
                        sample, target, bboxes, coord, ifflip=self.augtype['flip'], ifrotate=self.augtype['rotate'], ifswap=self.augtype['swap'])
            else:
...
#                crop = np.pad(crop,pad2,'constant',constant_values =self.pad_value)
        ...
        for i in range(3):
            newpad = self.crop_size[i] - crop.shape[1:][i]
            if newpad < 0:
                offset = -newpad / 2
                if i == 0:
                    crop = crop[:, offset:self.crop_size[i] + offset, :, :]
                elif i == 1:
                    crop = crop[:, :, offset:self.crop_size[i] + offset, :]
                else:
                    crop = crop[:, :, :, offset:self.crop_size[i] + offset]
                ...
                if i == 0:
                    pad2 = [[0, 0], [0, newpad], [0, 0], [0, 0]]
                elif i == 1:
                    pad2 = [[0, 0], [0, 0], [0, newpad], [0, 0]]
                else:
                    pad2 = [[0, 0], [0, 0], [0, 0], [0, newpad]]
                crop = np.pad(crop, pad2, 'constant', constant_values=self.pad_value)
