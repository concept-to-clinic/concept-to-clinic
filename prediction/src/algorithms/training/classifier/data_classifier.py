...
class DataBowl3Classifier(Dataset):
    def __init__(self, split, config, phase='train'):
        ...
        if phase != 'test':
            self.yset = np.array([labels[labels[:, 0] == f.split('-')[0].split('_')[0], 1]
                                  for f in split]).astype('int')
        idcs = [f.split('-')[0] for f in idcs]

        for idx in idcs:
            pbb = []
            pbb_file = os.path.join(bboxpath, idx + '_pbb.npy')
            # print(os.path.join(bboxpath,idx+'_pbb.npy'))
            ...
            lbb = []
            lbb_file = os.path.join(bboxpath, idx + '_lbb.npy')
            if os.path.exists(lbb_file):
                lbb = np.load(lbb_file)
            pbb_label = []
...
#             if idx.startswith()
    ...
    def __getitem__(self, idx, split=None):
        ...
        if self.phase != 'test':
            ...
            return torch.from_numpy(croplist).float(), torch.from_numpy(
                coordlist).float(), torch.from_numpy(isnodlist).int(), torch.from_numpy(y)
        else:
            return torch.from_numpy(croplist).float(), torch.from_numpy(
                coordlist).float(), torch.from_numpy(isnodlist).int()
...
class simpleCrop():
    ...
    def __call__(self, imgs, target):
        ...
        xx, yy, zz = np.meshgrid(np.linspace(normstart[0], normstart[0] +
                                             normsize[0], self.crop_size[0] /
                                             self.stride), np.linspace(normstart[1], normstart[1] +
                                                                       normsize[1], self.crop_size[1] /
                                                                       self.stride), np.linspace(normstart[2], normstart[2] +
                                                                                                 normsize[2], self.crop_size[2] /
                                                                                                 self.stride), indexing='ij')
