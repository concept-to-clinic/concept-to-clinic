...
#         weight = 3*torch.ones(y.size()).float().cuda()
        ...
        if args.output_classes == 1:
            ...
            # [n, 5] -> [n, 5, classes]
            isnod = isnod[:,:,None]
            isnod = isnod.expand(isnod.size()[0], isnod.size()[1], args.output_classes)
            y = y.view(-1)    # [n, 1] -> [n]
            y = Variable(y).long().cuda()
            loss2 = cross_entropy(casePred, y)
            sum = xsize[0] * xsize[1] * args.output_classes
        #loss2 = binary_cross_entropy(casePred,y[:,0])
